...
export function ProWaitlistForm() {
  ...
  return (
    <form onSubmit={submit} className="mt-6 flex flex-col gap-3 sm:flex-row">
      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="tu@email.com" className="min-h-12 flex-1 rounded-full border border-zinc-200 bg-white px-5 text-sm outline-none focus:border-[#ff6b00]" />
      <button disabled={status === "loading"} className="min-h-12 rounded-full bg-[#ff6b00] px-6 text-sm font-black text-white hover:bg-[#d95600] disabled:opacity-60">Quiero ser Pro</button>
